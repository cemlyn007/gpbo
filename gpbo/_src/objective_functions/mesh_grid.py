...
class MeshGridObjectiveFunction(core.ObjectiveFunction):
    ...
    def evaluate(self, key: jax.Array, *xs: jax.Array) -> jax.Array:
        ...
        if has_batch_dimension:
            if dense_xs.shape[1:] != self._xs.shape[1:]:
                raise ValueError(
                    "The input array must have the same shape as the mesh grid."
                )
        else:
        ...
        if has_batch_dimension:
            if self._xs.ndim == 1:
                indices = jnp.nonzero(
                    jnp.equal(self._xs, jnp.expand_dims(dense_xs, 1))
                )[1]
            else:
    ...
    def dataset_bounds(self) -> tuple[core.Boundary, ...]:
        ...
        if self._xs.ndim == 1:
            return tuple(
                core.Boundary(
                    min_x, max_x, dtype=self._dtype_to_python_type(self._xs.dtype)
                )
            ...
            return tuple(
                core.Boundary(
                    min_x[i], max_x[i], dtype=self._dtype_to_python_type(self._xs.dtype)
                )
    ...
    def _dtype_to_python_type(self, dtype) -> type:
        return {numpy.dtypes.Float32DType(): float, numpy.dtypes.Float64DType(): float}[
            dtype
        ]
